...
export class PayinService {
  ...
  async uploadOrder(
    ...
    const payinUsesBinanceParserRate = isParserFiatCode(dto.currency);
    ...
    try {
      ...
      const order = await this.prisma.$transaction(async (tx) => {
        ...
        // Лесенка процентов: тир по сумме заказа, fallback — плоская ставка трейдера.
        const traderPayinFrac = await this.traderRates.resolveFraction(
          requisite.traderId,
          'PAYIN',
          Number(dto.amount),
          Number(requisite.trader.payinRate),
        );
        const rtIn =
          payinUsesBinanceParserRate && parserRate !== undefined
            ? rateTraderIn(parserRate, traderPayinFrac)
            : null;
        const raIn =
          payinUsesBinanceParserRate && parserRate !== undefined
   ...
   * Requires a parser-backed fiat (RUB) Pay-In v2 rate snapshots and an assigned trader; no non-v2 settlement path.
   */
  private async creditBalancesOnPaid(
    ...
    if (
      !isParserFiatCode(order.currency.code) ||
      ...
      throw new BadRequestException(
        'Pay-In settlement requires a parser-backed fiat currency (RUB) with rate snapshots (rateTraderIn, rateAdminIn) and an assigned trader.',
      );
    }
    ...
    // Эффективная ставка трейдера — из снапшота ордера (лесенка применяет тир при назначении): rt = P*(1+frac).
    const traderPayinFrac = P > 0 ? rt / P - 1 : Number(traderProfile.payinRate);
  ...
  private async creditMerchantOnlyOnProviderPaid(
    ...
    if (
      ...
      throw new BadRequestException(
        'Provider Pay-In settlement requires a parser-backed fiat currency (RUB) with parser and admin rate snapshots.',
      );
    }
    const merchantFrac = percentToFraction(Number(order.commissionPercent));
    const merchantCredit = creditFiatMerchantPayin(paidAmountLocal, merchantFrac);
