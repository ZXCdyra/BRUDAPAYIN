...
export class ReferralService {
  ...
  async findAll(page = 1, limit = 20) {
    ...
    const [profiles, total] = await Promise.all([
      this.prisma.referralProfile.findMany({
        ...
        include: {
          user: { select: { id: true, login: true, isActive: true, createdAt: true } },
  ...
  async create(dto: CreateReferralDto) {
    ...
    if (existing) throw new ConflictException('Login already registered');

    const passwordHash = await hashPassword(dto.password);
    const currencyId = await this.currencies.requireActiveCurrencyIdByCode(
      (dto.currency ?? 'RUB').trim() || 'RUB',
    );

    const result = await this.prisma.$transaction(async (tx) => {
      const user = await tx.user.create({
        data: { login: dto.login, passwordHash, role: 'REFERRAL' },
      });
    ...
    this.logger.log(`Referral agent created: ${dto.login}`);
    return result;
  }
  ...
  async linkUser(referralProfileId: string, userId: string) {
    ...
    const updated = await this.prisma.user.update({
      ...
      select: { id: true, login: true, role: true, referredById: true },
    });
