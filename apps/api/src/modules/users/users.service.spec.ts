...
describe('UsersService', () => {
        ...
        expect.objectContaining({
          where: {
            AND: expect.arrayContaining([
              ...
              {
                OR: [
                  { login: { contains: 'test', mode: 'insensitive' } },
  ...
  describe('create', () => {
    it('creates nested merchant when role is MERCHANT', async () => {
      ...
      await service.create('merchant', 'password12345', UserRole.MERCHANT, {
    ...
    it('creates trader profile with optional balance and pool limits', async () => {
      ...
      await service.create('trader', 'password12345', UserRole.TRADER, {
    ...
    it('creates trader profile with Pay-In cascade fields when provided', async () => {
      ...
      await service.create('trader2', 'password12345', UserRole.TRADER, {
    ...
    it('rejects TRADER when pool min exceeds max', async () => {
      ...
      prisma.user.findUnique.mockResolvedValue(null);
      await expect(
        service.create('trader', 'password12345', UserRole.TRADER, {
          payoutMinLimit: 100,
          payoutMaxLimit: 50,
        }),
  ...
  describe('update', () => {
    ...
    it('allows other patches for OWNER', async () => {
      const { service, prisma } = createService();
      prisma.user.findUnique.mockResolvedValueOnce(ownerRow).mockResolvedValue(null);
    ...
    it('does not sync merchant lock when active merchant user gets email-only PATCH', async () => {
      ...
      prisma.user.findUnique.mockResolvedValueOnce(merchantUser).mockResolvedValue(null);
    ...
    it('keeps referral profile hook when user is already REFERRAL and email changes', async () => {
      ...
      prisma.user.findUnique.mockResolvedValueOnce(referralUser).mockResolvedValue(null);
