...
export class UsersService {
  ...
  private async backfillMissingReferralProfiles(rows: UserListWithInclude[]): Promise<UserListWithInclude[]> {
    ...
    const rubId = await this.currencies.requireActiveCurrencyIdByCode('RUB');
    await Promise.all(
      missing.map((u) =>
        this.prisma.referralProfile.upsert({
          where: { userId: u.id },
          create: { userId: u.id, referralPercent: 0, currencyId: rubId },
          update: {},
        }),
      ),
    );
  ...
  async create(
    ...
    if (opts?.email) {
      const existingEmail = await this.prisma.user.findUnique({ where: { email: opts.email } });
      if (existingEmail) {
    ...
    if (role === UserRole.REFERRAL) {
      const code = (opts?.referralCurrency ?? 'RUB').trim() || 'RUB';
    ...
    try {
      ...
      this.logger.log(`User ${login} created with role ${role}`);
  ...
  async update(
    id: string,
    data: { login?: string; email?: string; isActive?: boolean; merchantName?: string },
  ) {
    ...
    if (data.login !== undefined && data.login !== existing.login) {
      const conflict = await this.prisma.user.findUnique({ where: { login: data.login } });
      if (conflict) {
        throw new ConflictException('Login already registered');
      }
    }
    if (data.email && data.email !== existing.email) {
      const conflictEmail = await this.prisma.user.findUnique({ where: { email: data.email } });
      if (conflictEmail) {
        throw new ConflictException('Email already registered');
      }
    }
    ...
    if (updated.role === UserRole.REFERRAL) {
      ...
      await this.prisma.referralProfile.upsert({
        where: { userId: id },
        create: { userId: id, referralPercent: 0, currencyId: rubId },
        update: {},
      });
    }

    if (updated.role === UserRole.MERCHANT) {
      const merchantName = data.merchantName?.trim() || updated.login;
      try {
