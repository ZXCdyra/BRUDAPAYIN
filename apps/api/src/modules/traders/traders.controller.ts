...
import { UpsertTraderCommissionTiersDto } from './dto/upsert-commission-tiers.dto';
...
export class TradersController {
  ...
  @Get(':id/commission-tiers')
  @Roles(UserRole.ADMIN, UserRole.OWNER, UserRole.SUPPORT)
  @ApiOperation({ summary: 'Лесенка процентов трейдера (тиры по сумме заказа)' })
  @ApiQuery({ name: 'direction', required: false, enum: ['PAYIN', 'PAYOUT'] })
  getCommissionTiers(
    @Param('id', ParseUUIDPipe) id: string,
    @Query('direction') direction?: 'PAYIN' | 'PAYOUT',
  ) {
    return this.tradersService.getCommissionTiers(id, direction);
  }

  @Put(':id/commission-tiers')
  @Roles(UserRole.ADMIN, UserRole.OWNER)
  @ApiOperation({
    summary: 'Заменить лесенку процентов трейдера для направления (полная замена тиров)',
  })
  upsertCommissionTiers(
    @Param('id', ParseUUIDPipe) id: string,
    @Body() dto: UpsertTraderCommissionTiersDto,
  ) {
    return this.tradersService.upsertCommissionTiers(id, dto);
  }
