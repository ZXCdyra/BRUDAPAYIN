...
import { PARSER_FIAT_CODES } from '../exchange-rate/exchange-rate.service';
...
export class TradersService {
  ...
  /**
   * Block 5 В§4.4 вЂ” USDT capacity for Pay-In assignment and cabinet display.
   */
  async getUsdtWalletSummaryForUser(userId: string) {
    ...
    const pendingRows = await this.prisma.payinOrder.findMany({
      where: {
        ...
        currency: { code: { in: [...PARSER_FIAT_CODES] } },
      },
  ...
  /** Лесенка процентов трейдера: список тиров по направлению (или оба). */
  async getCommissionTiers(traderProfileId: string, direction?: 'PAYIN' | 'PAYOUT') {
    await this.ensureTraderProfile(traderProfileId);
    const tiers = await this.prisma.traderCommissionTier.findMany({
      where: { traderProfileId, ...(direction ? { direction } : {}) },
      orderBy: [{ direction: 'asc' }, { amountFrom: 'asc' }],
    });
    return {
      ...
      tiers: tiers.map((t) => ({
        ...
        amount_from: Number(t.amountFrom),
        amount_to: t.amountTo === null ? null : Number(t.amountTo),
        percent: Number(t.percent),
      })),
    };
  }

  /** Полная замена тиров одного направления (атомарно). */
  async upsertCommissionTiers(
    traderProfileId: string,
    dto: { direction: 'PAYIN' | 'PAYOUT'; tiers: { amountFrom: number; amountTo?: number | null; percent: number }[] },
  ) {
    await this.ensureTraderProfile(traderProfileId);

    const sorted = [...dto.tiers].sort((a, b) => a.amountFrom - b.amountFrom);
    for (const t of sorted) {
      if (t.amountTo !== null && t.amountTo !== undefined && t.amountTo <= t.amountFrom) {
        throw new BadRequestException('amount_to must be greater than amount_from');
      }
    ...
    for (let i = 1; i < sorted.length; i++) {
      ...
      if (prev.amountTo === null || prev.amountTo === undefined) {
        throw new BadRequestException('Only the last tier may have an open-ended range (amount_to = null)');
      }
      if (sorted[i].amountFrom < prev.amountTo) {
        throw new BadRequestException('Tier ranges must not overlap');
      }
    }

    const replaced = await this.prisma.$transaction(async (tx) => {
      await tx.traderCommissionTier.deleteMany({
        where: { traderProfileId, direction: dto.direction },
      });
      if (sorted.length > 0) {
        await tx.traderCommissionTier.createMany({
          data: sorted.map((t) => ({
            traderProfileId,
            direction: dto.direction,
            amountFrom: t.amountFrom,
            amountTo: t.amountTo ?? null,
            percent: t.percent,
          })),
        });
      }
      return tx.traderCommissionTier.findMany({
        where: { traderProfileId, direction: dto.direction },
    ...
    this.logger.log(
      `Trader ${traderProfileId} ${dto.direction} commission tiers replaced (${replaced.length} tiers)`,
    );

    return {
      traderId: traderProfileId,
      direction: dto.direction,
      tiers: replaced.map((t) => ({
        id: t.id,
        direction: t.direction,
        amount_from: Number(t.amountFrom),
        amount_to: t.amountTo === null ? null : Number(t.amountTo),
  ...
  private async ensureTraderProfile(traderProfileId: string) {
    const profile = await this.prisma.traderProfile.findUnique({
