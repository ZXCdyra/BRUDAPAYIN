...
describe('AuthService', () => {
  ...
  describe('login', () => {
    it('returns tokens when user is active and password matches', async () => {
      ...
      const res = await service.login('trader1', 'secret');

      expect(res).toEqual({
        accessToken: 'access',
        refreshToken: 'refresh',
        user: { id: 'u1', login: 'trader1', role: UserRole.TRADER },
      });
    });

    it('throws when login is unknown', async () => {
      const { service, prisma } = createService();
      prisma.user.findUnique.mockResolvedValue(null);

      await expect(service.login('nosuchuser', 'secret')).rejects.toThrow(
    ...
    it('throws when password does not match', async () => {
      ...
      await expect(service.login('trader1', 'wrong')).rejects.toThrow(
        new UnauthorizedException('Invalid credentials'),
      );
    });
    ...
    it('throws a deactivated message when password matches but user is inactive', async () => {
      ...
      await expect(service.login('trader1', 'secret')).rejects.toThrow(
