...
export class AuthService {
  ...
  async verify2FALogin(tempToken: string, code: string) {
    ...
    const user = await this.prisma.user.findUnique({
      ...
      select: { id: true, login: true, role: true, isActive: true, twoFaSecret: true },
    });
    ...
    this.logger.log(`User ${user.login} completed 2FA login`);
    return this.generateTokens(user);
  }

  private async generateTokens(user: { id: string; login: string; role: any }) {
    ...
    return { accessToken, refreshToken, user: { id: user.id, login: user.login, role: user.role } };
  }
