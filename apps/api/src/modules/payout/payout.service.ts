...
import { ExchangeRateService, isParserFiatCode } from '../exchange-rate/exchange-rate.service';
...
/**
 * Discriminated identity of a Pay-Out order assignee вЂ” standard trader (pool A) vs
...
export class PayoutService {
  ...
  constructor(
    ...
    private readonly traderRates: TraderRatesService,
  ...
  // в”Ђв”Ђв”Ђ External: order_upload в”Ђв”Ђв”Ђ
  ...
  async orderUpload(
    ...
    const isFiatV2 = isParserFiatCode(dto.currency);
  ...
  // в”Ђв”Ђв”Ђ External: order_info в”Ђв”Ђв”Ђ
  ...
  // в”Ђв”Ђв”Ђ External: info в”Ђв”Ђв”Ђ
  ...
  // в”Ђв”Ђв”Ђ Internal: getPool в”Ђв”Ђв”Ђ (PENDING orders without a trader; filtered by trader's payout limits)
  ...
  /**
   * Pool B вЂ” pending orders for the specialist's geo (country currency).
   */
  ...
  // в”Ђв”Ђв”Ђ Internal: traderTakeFromPool в”Ђв”Ђв”Ђ (trader self-assigns from pool; PENDING в†’ PROCESSING)
  ...
  // в”Ђв”Ђв”Ђ Internal: assignToTrader в”Ђв”Ђв”Ђ (admin/support assigns from pool to a trader or specialist)
  ...
  // в”Ђв”Ђв”Ђ Internal: getTraderOrders в”Ђв”Ђв”Ђ
  ...
  async getSpecialistNotifications(payoutTraderId: string, limit = 50) {
    ...
    for (const o of orders) {
      items.push({
        ...
        message: `${o.id} вЂ” ${Number(o.amount)} ${o.currency.code}`,
  ...
  // в”Ђв”Ђв”Ђ Internal: traderComplete в”Ђв”Ђв”Ђ
   ...
   * Symmetric for trader vs specialist failure paths (TZ вЂ” settlement on completion only).
   */
   ...
   * regardless of order status вЂ” traders need to be able to fix a wrong receipt during
  ...
  // в”Ђв”Ђв”Ђ Internal: traderFail в”Ђв”Ђв”Ђ
  ...
  // в”Ђв”Ђв”Ђ Private helpers в”Ђв”Ђв”Ђ

  /**
   * RISK NOTE: Pay-Out v2 specialist вЂ” same USDT credit formula as standard traders; balance on payout_traders.
   */
  ...
  /**
   * RISK NOTE: Fiat Pay-Out v2 вЂ” merchant was debited at order creation; credit trader USDT and book platform margin.
   */
  private async settlePayoutV2(tx: Prisma.TransactionClient, order: PayoutOrderScalars): Promise<void> {
    ...
    const P = Number(order.parserRate);
    const amountLocal = Number(order.amount);
    // Лесенка процентов: тир по сумме выплаты, fallback — плоская ставка трейдера.
    const traderPayoutFrac = await this.traderRates.resolveFraction(
      order.traderId,
      'PAYOUT',
      amountLocal,
      Number(trader.payoutRate),
    );
    const rateTraderOutVal = rateTraderOut(P, traderPayoutFrac);
    const rateAdminOutVal = Number(order.rateAdminOut);
    const creditUsdtVal = creditUsdtPayout(amountLocal, rateTraderOutVal);
    const marginUsdt = platformMarginUsdtPayout(amountLocal, rateAdminOutVal, rateTraderOutVal);
    const marginLocal = platformMarginLocal(marginUsdt, P);
    ...
    await tx.platformIncome.create({
      data: {
        ...
        rateTrader: rateTraderOutVal,
        rateAdmin: rateAdminOutVal,
        traderRatePct: traderPayoutFrac,
        merchantCommissionPct: merchantFrac,
        incomeUsdt: marginUsdt,
        incomeLocal: marginLocal,
      },
