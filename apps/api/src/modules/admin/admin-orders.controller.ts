...
const PAYOUT_ADMIN_ORDER_INCLUDE = {
  ...
  trader: { select: { id: true, user: { select: { login: true } } } },
  payoutTrader: { select: { user: { select: { login: true } } } },
...
export class AdminOrdersController {
  ...
  async findAll(
    ...
    if (isPayin) {
      ...
      if (traderFilter) {
        where.trader = {
          user: { login: { contains: traderFilter, mode: 'insensitive' } },
        };
      }
      ...
      return {
        data: orders.map((o) => ({
          ...
          traderName: o.trader?.user?.login ?? null,
  ...
  async findOne(
    ...
    if (isPayin) {
      ...
      if (!order) {
        ...
        const auditLogsFb = await this.prisma.auditLog.findMany({
          ...
          select: {
            ...
            createdAt: true,
            actorRole: true,
            actor: { select: { login: true, role: true } },
            oldValue: true,
            newValue: true,
          },
        });
      ...
      return {
        ...
        traderName: order.trader?.user?.login ?? null,
  ...
  private buildPayoutAdminDetail(
    ...
    auditLogs: Array<{
      ...
      actor: { login: string; role: string | null } | null;
      oldValue: unknown;
      newValue: unknown;
    }>,
  ) {
    ...
    const traderLabel =
      order.trader?.user.login ?? order.payoutTrader?.user.login ?? null;
    ...
    return {
      ...
      traderId: order.traderId,
      payoutTraderLogin: order.payoutTrader?.user.login ?? null,
      amount: Number(order.amount),
      currency: order.currency.code,
      status: order.status,
      applicationLogUiStatus: uiStatus,
      ...
      stakeholderAmounts: hideAssignmentSections
        ...
        : {
            ...
            trader: order.trader
              ...
              : order.payoutTrader
                ? {
                    label: order.payoutTrader.user.login,
