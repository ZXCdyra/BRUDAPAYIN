...
type SqlRow = {
  ...
  trader_standard_login: string | null;
  payout_specialist_login: string | null;
};
...
SELECT
  ...
  tu.login AS trader_standard_login,
  NULL::text AS payout_specialist_login
...
SELECT
  ...
  tu.login AS trader_standard_login,
  pu.login AS payout_specialist_login
...
`);
  ...
  private rowToDto(r: SqlRow): ApplicationLogListItemDto {
    ...
    if (uiStatus === ApplicationLogUiStatus.SUCCESS) {
      traderLabel =
        kind === 'PAYIN'
          ? r.trader_standard_login
          : r.trader_standard_login ?? r.payout_specialist_login;
    }
