...
export class SupportCabinetController {
  ...
  async getOrders(
    ...
    if (isPayout) {
      ...
      if (trader) {
        where.trader = { user: { login: { contains: trader, mode: 'insensitive' } } };
      }
      ...
      const [orders, total] = await Promise.all([
        this.prisma.payoutOrder.findMany({
          ...
          include: {
            ...
            trader: { include: { user: { select: { login: true } } } },
      ...
      return {
        data: orders.map((o) => ({
          ...
          traderName: o.trader?.user?.login ?? '',
  ...
  async getOrderDetails(@Param('id', ParseUUIDPipe) id: string) {
    ...
    if (payinOrder) {
      return {
        ...
        traderName: payinOrder.trader?.user?.login ?? '',
    ...
    if (payoutOrder) {
      return {
        ...
        traderName: payoutOrder.trader?.user?.login ?? '',
  ...
  async getDisputes(
    ...
    return {
      data: appeals.map((a) => ({
        ...
        traderName: a.payinOrder?.trader?.user?.login ?? '',
  ...
  async getDisputeDetails(@Param('id', ParseUUIDPipe) id: string) {
    ...
    return {
      ...
      traderName: appeal.payinOrder?.trader?.user?.login ?? '',
  ...
  async addNote(
    ...
    @CurrentUser('login') authorLogin: string,
  ) {
    const appeal = await this.prisma.appeal.findUnique({ where: { id } });
    if (!appeal) throw new NotFoundException(`Dispute ${id} not found`);
  ...
  async getBalances(
      ...
      const [merchants, total] = await Promise.all([
        this.prisma.merchant.findMany({
          ...
          include: {
            ...
            user: { select: { login: true, isActive: true } },
          },
          orderBy: { createdAt: 'desc' },
          take,
          skip,
    ...
    const where: Record<string, unknown> = {};
    if (search) {
      where.user = { login: { contains: search, mode: 'insensitive' } };
    }
