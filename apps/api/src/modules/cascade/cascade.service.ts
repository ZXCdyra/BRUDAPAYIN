...
export type CascadeTraderAssignment = {
  ...
  /** Distributed Redis lock held вЂ” release via CascadeRedisStateService after DB commit */
...
/** Alias for cascade ranking rows (materialized mirror in Redis per spec В§5вЂ“6). */
...
export class CascadeService {
   ...
   * Served from unified Redis payload when fresh (spec В§5.5).
   */
      ...
      if (enforceUsdtCapacity && parserRate !== undefined) {
        ...
        if (need > cap + 1e-9) {
          return {
            ...
            detail: `Required в‰€${need.toFixed(4)} USDT (with pay-in rate) exceeds trader capacity ${cap.toFixed(4)} USDT (balance + overdraft в€’ pending pay-in debits).`,
          };
        }
      }
      return { ok: true };
   ...
   * Fork autolimit-derived fields stored alongside rows in Redis (spec В§4.4, В§5.5).
   */
  async buildCurrencyPayload(
    ...
    if (isParserFiatCode(cur)) {
      try {
        parserRate = await this.exchangeRate.requireParserRateFiatPerUsdt(cur);
      } catch {
        parserRate = undefined;
      }
    }

    const enforceUsdt = isParserFiatCode(cur) && parserRate !== undefined;
  ...
  async listRequisiteRatingsForStaff(options: {
    ...
    for (const r of dbReqs) {
      ...
      if (q) {
        const login = (r.trader.user.login ?? '').toLowerCase();
        const masked = maskRequisiteNumber(r.number).toLowerCase();
        if (!login.includes(q) && !masked.includes(q) && !r.id.toLowerCase().includes(q)) {
          continue;
        }
      }
      ...
      rows.push({
        ...
        trader_label: r.trader.user.login ?? r.traderId,
    ...
    if (enforceUsdt) {
      ...
      for (const r of dbReqs) {
        if (!traderLabels.has(r.traderId)) {
          traderLabels.set(r.traderId, r.trader.user.login ?? r.traderId);
        }
      }
  ...
  async explainAssignmentOrder(
    ...
    const labelRows = reqIds.length
      ? await this.prisma.requisite.findMany({
          ...
          select: {
            id: true,
            number: true,
            trader: { select: { user: { select: { login: true } } } },
          },
        })
      : [];
    const labelById = new Map(
      labelRows.map((r) => [
        ...
        {
          requisite_masked: maskRequisiteNumber(r.number),
          trader_label: r.trader.user.login ?? '',
        },
      ]),
    );
  ...
  async updateSettings(
    data: {
      ...
      /** JSON array of `{ from, to, multiplier }` вЂ” Fork fill ladder (TZ В§7.3); null clears to code defaults. */
