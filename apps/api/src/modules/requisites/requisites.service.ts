...
export class RequisitesService {
  ...
  // в”Ђв”Ђв”Ђ Used by PayinService в”Ђв”Ђв”Ђ
  ...
  async findAvailable(currency: string, amount: number) {
    const currencyUsesBinanceParserRate = isParserFiatCode(currency);
    let parserRate: number | undefined;
    if (currencyUsesBinanceParserRate) {
      try {
        parserRate = await this.exchangeRate.requireParserRateFiatPerUsdt(currency);
      } catch {
        return null;
      }
    }
  ...
  async incrementUsageInTransaction(
    ...
    if (rows.length === 0) {
      ...
      throw new BadRequestException(
        'REQUISITE_LIMIT_EXCEEDED: This requisite cannot accept this assignment вЂ” total amount limit or operation cap would be exceeded.',
      );
    }
   ...
   * Values are clamped at zero вЂ” duplicate releases must not flip counters negative.
   */
  ...
  async releaseUsageInTransaction(
    ...
    if (nextOps !== rawNextOps || nextAmt !== rawNextAmt) {
      this.logger.warn(
        `Requisite ${requisiteId}: usage release clamped at zero (usedOps=${requisite.usedOps}в†’${nextOps}, usedAmount=${requisite.usedAmount}в†’${nextAmt}, releaseAmount=${amount})`,
      );
    }
  ...
  private async maybeAutoReenableAfterLimitReleaseInTransaction(
    ...
    if (conflicting) {
      this.logger.warn(
        `Requisite ${requisiteId}: skip auto-reenable after usage release вЂ” another active requisite shares this identity (other=${conflicting.id})`,
      );
      return;
    }
