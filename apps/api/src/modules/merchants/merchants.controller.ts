...
import { ResetMerchantBalancesDto } from './dto/reset-merchant-balances.dto';
...
export class MerchantsController {
  ...
  @Post(':id/balances/reset')
  @Roles(UserRole.OWNER)
  @ApiOperation({
    summary: 'Обнулить баланс мерчанта (только OWNER)',
    description:
      'Списывает текущие остатки по всем валютам (или указанной) проводками MANUAL_DEBIT и устанавливает балансы в ноль.',
  })
  @Audited(AuditAction.MERCHANT_BALANCE_RESET, AuditEntityType.Merchant)
  resetBalances(
    @Param('id', ParseUUIDPipe) id: string,
    @Body() dto: ResetMerchantBalancesDto,
  ) {
    return this.merchantsService.resetBalances(id, dto);
  }
}
