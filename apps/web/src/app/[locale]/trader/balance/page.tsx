...
import { ArrowDownCircle, ArrowUpCircle, DollarSign, MinusCircle, Copy, Check } from 'lucide-react';
...
export default function BalanceHistoryPage() {
  ...
  const copyToClipboard = useCallback(async (text: string, setter: React.Dispatch<React.SetStateAction<boolean>>) => {
    try {
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        header: t('colBy'),
        render: (tx: BalanceTx) => (
          <span className="text-xs text-text-muted">{tx.createdBy?.login ?? t('system')}</span>
        ),
      },
      {
        key: 'createdAt',
      ...
      >
        ...
        <div className="space-y-2 border-t border-border-subtle pt-4">
          ...
          {wallet?.usdt_trc20_deposit_address ? (
            <div className="flex items-center gap-2">
              <p className="flex-1 break-all rounded-lg bg-bg-primary/50 p-2 font-mono text-xs text-text-secondary">
                {wallet.usdt_trc20_deposit_address}
              </p>
              <button
                type="button"
                onClick={() => copyToClipboard(wallet.usdt_trc20_deposit_address!, setCopiedTrc)}
                className={`shrink-0 flex h-8 items-center gap-1.5 rounded-lg px-2.5 text-xs font-medium transition-all duration-200 ${
                  copiedTrc
        ...
        <div className="space-y-2 border-t border-border-subtle pt-4">
          ...
          {wallet?.usdt_erc20_deposit_address ? (
            <div className="flex items-center gap-2">
              ...
              <button
                type="button"
                onClick={() => copyToClipboard(wallet.usdt_erc20_deposit_address!, setCopiedErc)}
                className={`shrink-0 flex h-8 items-center gap-1.5 rounded-lg px-2.5 text-xs font-medium transition-all duration-200 ${
                  copiedErc
                    ? 'bg-success-muted text-success'
                    : 'bg-surface-tertiary text-text-secondary hover:bg-surface-elevated hover:text-text-primary'
