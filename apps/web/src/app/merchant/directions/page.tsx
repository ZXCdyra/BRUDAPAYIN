    ...
    <div className="space-y-8 animate-fade-in">
      <div>
        <h1 className="text-2xl font-bold text-text-primary flex items-center gap-2">
          <Percent size={24} />
          Направления и комиссии
        </h1>
        <p className="text-sm text-text-muted mt-1">
          Параметры и комиссии ваших направлений приёма и выплат (только просмотр).
        </p>
      </div>
      ...
      <div className="flex gap-3 rounded-xl border border-border-primary bg-bg-secondary/50 p-4 text-sm text-text-secondary">
        ...
        <p>
          Условия задаёт платформа. Если нужны изменения — обратитесь к своему менеджеру или в
          поддержку.
        </p>
      </div>
      ...
      {showCustom && (
        <div className="rounded-lg border border-border-primary/60 bg-bg-secondary/30 px-3 py-2 text-xs text-text-muted">
          Показаны <span className="text-text-secondary font-medium">персональные условия</span>,
          настроенные для вашего аккаунта.
        </div>
      )}
      ...
      {!showCustom &&
        ...
        platformDirections.length > 0 && (
          <div className="rounded-lg border border-border-primary/60 bg-bg-secondary/30 px-3 py-2 text-xs text-text-muted">
            Персональных условий нет. Ниже —{' '}
            <span className="text-text-secondary font-medium">стандартные направления платформы</span>.
          </div>
        )}
        ...
        customDirections.map((dir) => {
          ...
          >
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-3 text-sm">
              <div>
                <p className="text-text-muted text-xs mb-1">Диапазон сумм</p>
              ...
              <div>
                <p className="text-text-muted text-xs mb-1">Базовая комиссия</p>
                <p className="text-text-primary font-mono">
                  {num(dir.defaultCommissionPercent).toFixed(2)}%
                </p>
              </div>
              <div>
                <p className="text-text-muted text-xs mb-1">Лесенка комиссий</p>
            ...
            {dir.commissionTiers.length > 0 && (
              <div className="border-t border-border-primary pt-4">
                <p className="text-xs text-text-muted mb-2">Лесенка комиссий</p>
      ...
      {!loading &&
        ...
        platformError && (
          <div className="rounded-xl border border-border-primary bg-bg-card p-8 text-center text-sm text-text-muted">
            Не удалось загрузить стандартные направления. Попробуйте позже или обратитесь в поддержку.
          </div>
        )}

      {!loading && !showCustom && !platformError && platformDirections.length === 0 && (
        <div className="rounded-xl border border-border-primary bg-bg-card p-8 text-center text-sm text-text-muted">
          На платформе пока не настроено ни одного направления. Обратитесь в поддержку.
        </div>
      )}
        ...
        platformDirections.map((dir) => (
          ...
          >
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-3 text-sm">
              ...
              <div>
                <p className="text-text-muted text-xs mb-1">Комиссия</p>
