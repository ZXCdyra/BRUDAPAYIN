...
export default function LoginPage() {
  ...
  const { login: doLogin, verify2FA, requires2FA, isAuthenticated, user, loadUser, isLoading } = useAuth();
  ...
  async function handleLogin(e: React.FormEvent) {
    e.preventDefault();
    setFieldErrors({});
    const parsed = loginCredentialsSchema.safeParse({ login, password });
    if (!parsed.success) {
    ...
    <div className="relative flex min-h-screen items-center justify-center bg-bg-primary p-4">
      ...
      <div className="relative w-full max-w-md animate-fade-in">
        <div className="mb-8 text-center">
          ...
          <h1 className="text-2xl font-bold text-text-primary">BrudaPay</h1>
        ...
        <p className="mt-6 text-center text-xs text-text-muted">
          BrudaPay &middot; All rights reserved
        </p>
      </div>
    </div>
  );
