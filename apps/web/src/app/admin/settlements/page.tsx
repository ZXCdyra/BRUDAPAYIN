...
interface SettlementRow {
  ...
  trader: { user: { login?: string; email?: string | null } } | null;
  payoutTrader: { user: { login?: string; email?: string | null } } | null;
...
function participantLabel(row: SettlementRow): string {
  ...
  const payout = row.payoutTrader?.user?.login ?? row.payoutTrader?.user?.email;
  if (payout) return `Pay-Out specialist: ${payout}`;
  const trader = row.trader?.user?.login ?? row.trader?.user?.email;
  if (trader) return `Trader: ${trader}`;
  return '—';
}
...
export default function SettlementsPage() {
  ...
  const { data: payoutOpts } = useQuery<{
    data: Array<{ id: string; login?: string; email?: string | null }>;
  }>({
  ...
  const participantPick =
    participantRole === 'any' ? null : participantRole === 'trader' ? (
      ...
      <Select
        label="Логин специалиста"
        options={[
          { value: '', label: '—' },
          ...payoutSpecialists.map((p) => ({ value: p.id, label: p.login ?? p.email ?? '—' })),
        ]}
        value={participantId}
        onChange={(e) => {
          setParticipantId(e.target.value);
