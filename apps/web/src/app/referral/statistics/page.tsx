      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <div className="rounded-xl border border-border-primary bg-bg-secondary p-4">
          ...
          <p className="mt-1 text-xl font-bold text-accent-green">
            {formatCurrency(data?.balance ?? 0, data?.currency ?? 'RUB')}
          </p>
...
function TraderCard({ trader }: { trader: TraderStat }) {
  ...
  const primaryFiatBalanceRow = normBalances.find((b) => b.code.toUpperCase() === 'RUB');
  ...
  return (
    <div className="rounded-lg border border-border-primary bg-bg-secondary/50 p-4">
      <div className="flex items-start justify-between">
        <div>
          <p className="font-medium text-text-primary">{trader.login ?? trader.email ?? '—'}</p>
      ...
      <div className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-4">
        <Metric
          ...
          value={formatCurrency(primaryFiatBalanceRow?.amount ?? 0, primaryFiatBalanceRow?.code ?? 'RUB')}
        />
        <Metric
          icon={<Wallet className="h-4 w-4 text-text-muted" />}
          label="Balance USDT"
        ...
        <Metric
          ...
          value={`${trader.completedPayins} / ${formatCurrency(trader.totalPayinAmount, 'RUB')}`}
        />
        <Metric
          icon={<ArrowUpFromLine className="h-4 w-4 text-accent-blue" />}
          label="Pay-Out (completed)"
          value={`${trader.completedPayouts} / ${formatCurrency(trader.totalPayoutAmount, 'RUB')}`}
        />
      </div>
    </div>
  );
    ...
    <div className="rounded-lg border border-border-primary bg-bg-secondary/50 p-4">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-xs text-text-muted">{merchant.login ?? merchant.email ?? '—'}</p>
        </div>
        <div className="flex gap-2">
