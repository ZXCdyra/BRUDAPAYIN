...
interface ReferralProfile {
  ...
  currency: string;
  createdAt: string;
  user: { login?: string; email?: string | null; isActive: boolean };
  referrals: {
    id: string;
    login?: string;
    email?: string | null;
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-text-muted">{profile?.user.login ?? profile?.user.email ?? '—'}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard
          ...
          label="My Balance"
          value={formatCurrency(profile?.balance ?? 0, profile?.currency ?? 'RUB')}
          loading={isLoading}
          accent="green"
        />
        <StatCard
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border-primary">
                  <th className="pb-3 text-left text-xs font-medium text-text-muted">Логин</th>
              ...
              <tbody className="divide-y divide-border-primary">
                {profile.referrals.map((user) => (
                  <tr key={user.id} className="hover:bg-bg-secondary/50 transition-colors">
                    <td className="py-3 text-text-primary">{user.login ?? user.email ?? '—'}</td>
                    <td className="py-3">
