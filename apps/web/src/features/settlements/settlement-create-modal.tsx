...
export function SettlementCreateModal({
  ...
  const [merchantCurrency, setMerchantCurrency] = useState('RUB');
  ...
  const { data: traders = [] } = useQuery<TraderOption[]>({
    ...
    queryFn: async () => {
      ...
      return res.data.map((t) => ({
        id: t.id,
        name: t.user.login ?? t.user.email ?? '—',
      }));
    },
          ...
          <>
            ...
            <Select
              ...
              options={[
                ...
                ...payoutOptions.map((p) => ({
                  value: p.id,
                  label: `${p.login ?? p.email ?? '—'} (${p.balance_usdt.toFixed(2)} USDT)`,
                })),
              ]}
