...
type MetaResponse = {
  ...
  traders: { id: string; login?: string; email?: string | null }[];
  currencies: string[];
  errorCodes: string[];
};
...
export function OrdersLogsPageView({
  ...
  const chips = useMemo(() => {
    ...
    if (filters.traderId && meta) {
      ...
      push('trader', `Trader: ${t?.login ?? t?.email ?? filters.traderId}`, () =>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          ...
          <Select
            ...
            options={[
              ...
              ...meta.traders.map((t) => ({ value: t.id, label: t.login ?? t.email ?? '—' })),
            ]}
