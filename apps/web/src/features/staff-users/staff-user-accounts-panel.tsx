...
export function StaffUserAccountsPanel({ queryKeyPrefix }: StaffUserAccountsPanelProps) {
  ...
  const [pendingBalanceReset, setPendingBalanceReset] = useState<{
    merchantId: string;
    login: string;
  } | null>(null);
  ...
  const createUser = useMutation({
    mutationFn: async (payload: typeof form) => {
      ...
      if (payload.role === UserRole.REFERRAL) {
        ...
        body.referralCurrency = payload.referralCurrency.trim() || 'RUB';
      }
  ...
  const resetMerchantBalances = useMutation({
    mutationFn: (merchantId: string) =>
      api.post<{ message?: string }>(internalPaths.merchantBalanceReset(merchantId), {}),
    onSuccess: () => {
      setPendingBalanceReset(null);
      invalidateDirectory();
    },
  });
  ...
  const columns = [
    {
      ...
      render: (u: DirectoryUser) => (
        <div>
          <p className="font-medium text-text-primary">{u.login ?? u.email ?? '—'}</p>
    ...
    },
    {
      ...
      render: (u: DirectoryUser) => (
        ...
        >
          ...
          {isOwnerCabinet && u.role === UserRole.MERCHANT && u.merchant ? (
            <IconButton
              label="Обнулить баланс мерчанта"
              variant="danger"
              onClick={() =>
                setPendingBalanceReset({ merchantId: u.merchant!.id, login: u.login ?? '' })
              }
            >
          ...
          {u.role === UserRole.MERCHANT && !u.merchant ? (
            <Button
              ...
              onClick={() => {
                setMerchantProfileCreateModal({ userId: u.id, login: u.login ?? u.email ?? '' });
                setMerchantProfileCreateName('');
              }}
            >
  ...
  return (
    <div className="space-y-4">
      <ConfirmDialog
        ...
        description={
          <>
            <span className="font-medium text-text-primary">{form.login.trim()}</span>
            {' · '}
      ...
      <ConfirmDialog
        open={!!pendingBalanceReset}
        onOpenChange={(next) => !next && setPendingBalanceReset(null)}
        tone="danger"
        title="Обнулить баланс мерчанта?"
        description={
          pendingBalanceReset ? (
            <>
              Все остатки по валютам будут списаны проводками MANUAL_DEBIT и установлены в ноль для{' '}
              <span className="font-medium text-text-primary">{pendingBalanceReset.login}</span>.
              Действие необратимо, история проводок сохранится.
            </>
          ) : null
        }
        confirmLabel="Обнулить баланс"
        cancelLabel="Отмена"
        loading={resetMerchantBalances.isPending}
        onConfirm={() => {
          if (!pendingBalanceReset) return;
          resetMerchantBalances.mutate(pendingBalanceReset.merchantId);
        }}
      />
        ...
        description={
          pendingStatusToggle ? (
            <>
              ...
              <span className="font-medium text-text-primary">{pendingStatusToggle.login}</span>
            </>
          ) : null
        }
        confirmLabel={
        ...
        description={
          pendingPermanentDelete ? (
            <>
              ...
              <span className="font-medium text-text-primary">{pendingPermanentDelete.login}</span>
            </>
          ) : null
        }
        ...
        {merchantProfileCreateModal ? (
          ...
          >
            <p className="text-sm text-text-muted">
              Link a payment profile to <strong>{merchantProfileCreateModal.login}</strong>. After
      ...
      <ReferralAgentManageModal
        ...
        agentLogin={referralManage?.login ?? ''}
