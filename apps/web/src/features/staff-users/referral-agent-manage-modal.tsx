...
interface ReferralAgentDetail {
  ...
  user: { id: string; login?: string; email?: string | null; isActive: boolean; createdAt: string };
  ...
  return (
    <>
      <Modal
        ...
        title={`Referral agent — ${agentLogin}`}
        size="lg"
      >
        {formError ? (
          ...
          <div className="space-y-4 max-h-[70vh] overflow-y-auto pr-1">
            <div className="text-sm">
              <p>
                ...
                <span className="font-medium text-text-primary">{detail.user.login ?? detail.user.email ?? '—'}</span>
              </p>
              ...
              {detail.referrals.length === 0 ? (
                ...
                <ul className="text-sm space-y-2">
                  {detail.referrals.map((u) => (
                    ...
                    >
                      <span className="text-text-primary truncate">{u.login ?? u.email ?? '—'}</span>
      ...
      <ConfirmDialog
        ...
        description={
          pendingUnlink ? (
            <>
              <span className="font-medium text-text-primary">{pendingUnlink.login ?? pendingUnlink.email ?? '—'}</span> will no longer count toward
              this referral agent.
            </>
          ) : null
        }
