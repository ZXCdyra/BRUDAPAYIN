      ...
      <div className="flex h-16 items-center justify-between border-b border-border-primary px-4">
        {!collapsed && (
          <div className="flex items-center gap-2.5">
            <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-accent-blue font-bold text-white text-[10px]">
              Br
            </div>
            <div>
              <p className="text-sm font-semibold text-text-primary">Trader</p>
              <p className="text-xs text-text-muted">BrudaPay</p>
            </div>
          </div>
        )}
        <button
      ...
      <div className="border-t border-border-primary p-3">
        {!collapsed && user && (
          <div className="mb-2 px-2">
            <p className="text-sm text-text-primary truncate">{user.login ?? user.email ?? '—'}</p>
