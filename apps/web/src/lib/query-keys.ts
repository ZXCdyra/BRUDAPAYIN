...
/** Top-up requests (trader + admin). */
export const topupKeys = {
  scope: () => ['topup-requests'] as const,
  myRequests: (page: number, status?: string) =>
    [...topupKeys.scope(), 'my', page, status ?? 'all'] as const,
  adminList: (page: number, status?: string) =>
    [...topupKeys.scope(), 'admin', page, status ?? 'all'] as const,
};
