...
export type SettlementListRow = {
  ...
  trader: { id?: string; user: { login?: string; email?: string | null } } | null;
  payoutTrader: { id?: string; user: { login?: string; email?: string | null } } | null;
  ...
  return {
    ...
    trader:
      raw.trader && raw.trader.user
        ? {
            id: raw.trader.id,
            user: { login: raw.trader.user.login, email: raw.trader.user.email },
          }
        : null,
    payoutTrader:
      raw.payoutTrader && raw.payoutTrader.user
        ? {
            id: raw.payoutTrader.id,
            user: { login: raw.payoutTrader.user.login, email: raw.payoutTrader.user.email },
          }
        : null,
