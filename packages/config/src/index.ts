...
export const config = {
  ...
  rapira: {
    baseUrl: optional('RAPIRA_BASE_URL', 'https://api.rapira.net'),
    /** Set false to disable Rapira polling (e.g. on secondary processes). */
    pollEnabled: optional('RAPIRA_POLL_ENABLED', 'true') === 'true',
    /** Respect Rapira public limits: 5 rps / 100 rpm. */
    pollMs: parseInt(optional('RAPIRA_POLL_MS', '15000'), 10),
    redisKey: optional('RAPIRA_REDIS_KEY', 'rapira:usdt_rub'),
    /** Which Rapira price feeds the parser rate P (fiat per 1 USDT): mid | bid | ask | close. */
    priceMode: optional('RAPIRA_PRICE_MODE', 'mid') as 'mid' | 'bid' | 'ask' | 'close',
    /** If no successful Rapira refresh for this many minutes, log warn and optionally notify owner. */
    staleAlertMinutes: parseInt(optional('RAPIRA_STALE_ALERT_MINUTES', '15'), 10),
  },
  ownerOps: {
